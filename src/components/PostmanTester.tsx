...
const METHOD_COLORS: Record<HttpMethod, { bg: string; text: string; border: string }> = {
  GET: { bg: "bg-green-500", text: "text-green-500", border: "border-green-500" },
  POST: { bg: "bg-yellow-500", text: "text-yellow-500", border: "border-yellow-500" },
  PUT: { bg: "bg-blue-500", text: "text-blue-500", border: "border-blue-500" },
  PATCH: { bg: "bg-purple-500", text: "text-purple-500", border: "border-purple-500" },
  DELETE: { bg: "bg-red-500", text: "text-red-500", border: "border-red-500" },
};
...
export function PostmanTester() {
  ...
  useEffect(() => {
    if (currentEndpoint && typeof window !== "undefined") {
      setCustomUrl(`${window.location.origin}/api/mock/${currentEndpoint.slug}`);
    }
  }, [currentEndpoint]);
      ...
      <div className="bg-slate-800 p-4 border-b border-slate-700">
        <div className="flex gap-2">
          ...
          <div className="relative">
            ...
            >
              {selectedMethod}
              <ChevronDown className={cn("w-4 h-4 transition-transform", showMethodDropdown && "rotate-180")} />
            </button>
          ...
          <button
            onClick={handleTest}
            disabled={isLoading}
            className="px-6 py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-500/50 text-white font-bold rounded-lg flex items-center gap-2 transition-all hover:shadow-lg"
          >
            {isLoading ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-4 text-sm">
                <label className="flex items-center gap-2 text-slate-300">
                  <input type="radio" name="bodyType" defaultChecked className="text-orange-500 focus:ring-orange-500" />
                  raw
                </label>
                <span className="px-3 py-1 bg-slate-700 rounded text-orange-400 text-xs font-mono">JSON</span>
              </div>
              <textarea
                value={requestBody}
